...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate 2.5D T1 MRI denoising on held-out cases.")
...
def resize_stack(noisy: np.ndarray, slice_idx: int, context: int, target_size: int) -> torch.Tensor:
    depth = noisy.shape[2]
    stack = []
    for offset in range(-context, context + 1):
        idx = min(max(slice_idx + offset, 0), depth - 1)
        stack.append(noisy[:, :, idx])
    tensor = torch.from_numpy(np.stack(stack, axis=0)).float().unsqueeze(0)
    return F.interpolate(tensor, size=(target_size, target_size), mode="bilinear", align_corners=False)
...
def predict_case(
    ...
    with torch.no_grad():
        for idx in indices:
            noisy_t = resize_stack(noisy, idx, context, target_size).to(device)
            clean_t = resize_slice(clean[:, :, idx], target_size)
            pred_t = model(noisy_t).cpu()
            noisy_slices.append(noisy_t.cpu().numpy()[0, context])
...
def main() -> None:
    ...
    context = int(ckpt_args.get("context", args.context))
    model = ResidualUNet2D(in_channels=2 * context + 1, out_channels=1, base_channels=base_channels).to(device)
    ...
    for fig_count, record in enumerate(tqdm(records), start=0):
        ...
        noisy_eval, pred_eval, clean_eval = predict_case(model, noisy, clean, indices, target_size, context, device)
