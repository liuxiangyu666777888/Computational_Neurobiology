...
The results support two conclusions. First, residual U-Nets are effective baselines for paired T1 MRI denoising: every trained model reduces MAE and MSE and improves PSNR and SSIM compared with the noisy input. Second, 2.5D context is useful but does not require a full 3D model. The 2.5D model has nearly the same computational cost as the 2D model while giving the best PSNR and SSIM in this experiment.

The 3D result should be interpreted as a practical baseline rather than a fully optimized volumetric system. Full-volume 3D inference is expensive, so the implemented model uses patch training and center-slice prediction with fixed depth context during evaluation. This reduces memory use but also limits global anatomical context and may introduce train-test differences. A stronger 3D experiment would use more patches per case, longer training, and overlap-averaged sliding-window inference over the whole volume.
